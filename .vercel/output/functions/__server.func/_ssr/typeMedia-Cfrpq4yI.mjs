//#region node_modules/.nitro/vite/services/ssr/assets/typeMedia-Cfrpq4yI.js
...
var RV_CARD_MEDIA = LIFESTYLE.classADiesel;
var RV_TYPE_MEDIA = {
...
function classAIsGas(type, fuel) {
...
function mediaForRvType(type, fuelType, chassis) {
	const t = (type || "").toLowerCase();
	const ch = (chassis || "").toLowerCase();
	if (!t && !ch) return RV_CARD_MEDIA;
	if (/toy\s*hauler/.test(t)) return RV_TYPE_MEDIA.toyHauler;
	if (/fifth\s*wheel|5th\s*wheel|fiver|\bfifth\b/.test(t)) return RV_TYPE_MEDIA.fifthWheel;
	...
	if (/super\s*c/.test(t) || superCChassis && !/class\s*a/.test(t)) return RV_TYPE_MEDIA.superC;
	if (/class\s*c/.test(t)) return RV_TYPE_MEDIA.classC;
	if (/class\s*a|diesel\s*pusher|gas\s*pusher|motor\s*home|motorhome/.test(t)) return classAIsGas(t, fuelType) ? RV_TYPE_MEDIA.classAGas : RV_TYPE_MEDIA.classADiesel;
	if (/diesel|gas/.test(t) && /class/.test(t)) return classAIsGas(t, fuelType) ? RV_TYPE_MEDIA.classAGas : RV_TYPE_MEDIA.classADiesel;
	return RV_CARD_MEDIA;
}
function resolveCardImage(spec) {
	return mediaForRvType(spec.type, spec.fuelType, spec.chassis) || spec.image || RV_CARD_MEDIA;
}
//#endregion
export { mediaForRvType as n, resolveCardImage as r, RV_CARD_MEDIA as t };
