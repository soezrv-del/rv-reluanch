...
import { $ as Funnel, A as Plus, C as Search, Ct as BedDouble, Q as GitCompare, St as Bookmark, T as Ruler, U as LoaderCircle, W as List, Z as Heart, _t as Check, d as Trash2, et as Fuel, gt as ChevronDown, ht as ChevronLeft, j as Pencil, l as TriangleAlert, pt as ChevronUp, q as Keyboard, t as X, v as Sparkles, w as ScanLine, wt as ArrowUpLeft } from "../_libs/lucide-react.mjs";
import { c as hapticSnap, d as cn, r as useKeyboardInset } from "./routes-Pmuw5ThC.mjs";
import { i as SuiteBackdrop, n as SHARED_PRESTIGE_BACKDROP, o as useAdaptiveGlass, r as ScrollSuiteHeader, s as usePullToReset, t as PullResetHint } from "./SuitePage-BuhYUyxz.mjs";
import { C as rvClassLabel, a as applyCascadeChange, b as modelYearWindow, c as compareSelectionKey, h as getModelsForYearMake, i as YEARS, l as countModelsForClass, m as getMakesForYear, n as RV_CLASS_TABS, o as buildCascadeOptions, r as RV_DATA, t as MAKES, v as modelAvailableInYear, w as searchCatalog, x as ratingFor, y as modelPickerMeta } from "./catalog-cr78mWmG.mjs";
import { r as resolveCardImage } from "./typeMedia-CmX7-SFY.mjs";
import { t as SelectSheet } from "./SelectSheet-Ctbg8Ig5.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/RvFaxApp-CHqoKcpV.js
...
function WizardWheel({ title, subtitle, items, selected, onSelect, emptyHint = "No options for this step", allowCustom = false, customLabel = "Use custom entry", customPlaceholder = "Type your own…", stepIndex = 0, stepCount = 4, stepLabels, mode, onModeChange, hideModeTabs = false }) {
	...
	const dragStartClientX = (0, import_react.useRef)(0);
	...
	(0, import_react.useEffect)(() => {
		...
		if (!el) return;
		const begin = (clientX, clientY, mode) => {
			stopRaf();
		...
		const indexFromTap = (clientX, clientY, len) => {
			const vv = window.visualViewport;
			const ys = [clientY];
			const xs = [clientX];
			if (vv) {
				if (vv.offsetTop) ys.push(clientY - vv.offsetTop, clientY + vv.offsetTop);
				if (vv.offsetLeft) xs.push(clientX - vv.offsetLeft, clientX + vv.offsetLeft);
				const chrome = window.innerHeight - vv.height - vv.offsetTop;
				if (Math.abs(chrome) > 8) ys.push(clientY - chrome, clientY + chrome);
			}
			for (const x of xs) for (const y of ys) {
				const row = document.elementFromPoint(x, y)?.closest?.("[data-wheel-index]");
			...
			if (content) {
				...
				for (const y of ys) {
					const rel = y - box.top;
					if (rel >= 0 && rel < len * ROW_H) return clampIndex(Math.floor(rel / ROW_H), len);
				}
			}
			const trackBox = el.getBoundingClientRect();
			for (const y of ys) {
				const rel = y - trackBox.top;
				if (rel >= 124 && rel < 170) return focusIdxRef.current;
			}
		...
		const end = (clientY) => {
			...
			samplesRef.current = [];
			if (!movedRef.current) {
				const idx = indexFromTap(dragStartClientX.current, dragStartClientY.current, len);
...
var RvDetail = (0, import_react.lazy)(() => import("./RvDetail-DPGvoVxv.mjs").then((m) => ({ default: m.RvDetail })));
var RvCompare = (0, import_react.lazy)(() => import("./RvCompare-BLMRrvNU.mjs").then((m) => ({ default: m.RvCompare })));
var VinDecoder = (0, import_react.lazy)(() => import("./VinDecoder-BuwbmHal.mjs").then((m) => ({ default: m.VinDecoder })));
										...
										/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											...
											children: [
												...
												wizardStep === "type" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(WizardWheel, {
													...
													subtitle: year ? `${typeItems.length - 1} classes in ${year} · pick one to narrow brands` : "Class A diesel, Class A gas, B, C, Super C, fifth wheel, trailer, toy hauler",
													items: typeItems,
													selected: rvType,
