 ...
 *
 * Brochure / OEM pins always win. Lot numbers are lot data — they only paint
 * when the brochure row is blank or "Confirm brochure". Catalog digit
 * heuristics (33'6" from 337RLS, series sleeps 8) may be replaced when
 * dataSource is catalog/estimated. 0 / null / tank-count values stay blank.
 ...
 */
export function lotPublishedFromRow(row: Record<string, unknown> | null | undefined): LotPublishedSpecs {
  ...
  return {
    ...
    dryWeightLbs: pickNum(
      row,
      ["dry_weight", "dryWeight", "uvw", "uvw_lbs", "uvwLbs"],
      "weight",
    ),
    hitchLbs: pickNum(
      row,
      ["hitch_weight", "hitchWeight", "hitch_lbs", "hitchLbs"],
      "weight",
    ),
    payloadLbs: pickNum(
      row,
      ["payload", "standard_payload", "ccc", "ccc_lbs"],
      "weight",
    ),
    lengthFt: pickNum(
      row,
      ["vehicle_body_length", "length_ft", "length", "lengthFt"],
      "weight",
    ),
    heightFt: pickNum(
      row,
    ...
    widthFt: pickNum(
      row,
      ["vehicle_body_width", "width_ft", "width", "exterior_width"],
      "weight",
    ),
 ...
 * Catalog digit heuristics may move. OEM-year pins stay.
 */
export function applyLotSpecsToBrochure(
  ...
  const paint = (
    ...
    if (!isFactsGap(cur) && !(replaceGuess && guess)) return;
    (next as unknown as Record<string, unknown>)[key] = value;
    filled.push(label);
  };

  if (lot.gvwrLbs != null) {
    paint("GVWR", "gvwr", fmtLbs(lot.gvwrLbs));
    if (isFactsGap(next.gvwrLbs)) next.gvwrLbs = Math.round(lot.gvwrLbs);
  }
  if (lot.dryWeightLbs != null) {
    paint("UVW", "uvw", fmtLbs(lot.dryWeightLbs));
    if (isFactsGap(next.uvwLbs)) next.uvwLbs = Math.round(lot.dryWeightLbs);
  }
  if (lot.hitchLbs != null) {
    paint("HITCH", "hitchOrPin", fmtLbs(lot.hitchLbs));
  }
  if (lot.payloadLbs != null && isFactsGap(next.ccc)) {
    next.ccc = fmtLbs(lot.payloadLbs);
    if (isFactsGap(next.cccLbs)) next.cccLbs = Math.round(lot.payloadLbs);
    filled.push("CCC");
  }
  if (lot.lengthFt != null) {
    paint("LENGTH", "lengthFt", fmtFt(lot.lengthFt), true);
    if (isFactsGap(next.lengthIn) || guess) next.lengthIn = fmtFt(lot.lengthFt);
  }
  if (lot.heightFt != null) {
    paint("HEIGHT", "exteriorHeight", fmtFt(lot.heightFt), true);
  }
  // Coast often stores width as a rounded 8. Do not replace 8'6" with 8'0".
  if (lot.widthFt != null && Math.abs(lot.widthFt - 8) > 0.2) {
    paint("WIDTH", "exteriorWidth", fmtFt(lot.widthFt), true);
  }
  if (lot.sleeps != null) {
    paint("SLEEPS", "sleeps", String(lot.sleeps), true);
  }
  if (lot.slides != null) {
    paint("SLIDES", "slideouts", String(lot.slides), true);
  }
  if (lot.freshGal != null) paint("FRESH WATER", "freshWater", fmtGal(lot.freshGal));
  if (lot.grayGal != null) paint("GRAY WATER", "grayWater", fmtGal(lot.grayGal));
  if (lot.blackGal != null) paint("BLACK WATER", "blackWater", fmtGal(lot.blackGal));

  const propane = fmtPropane(
    lot.propaneLbs ?? null,
    lot.propaneGal ?? null,
  );
  if (propane) paint("PROPANE", "propane", propane);
