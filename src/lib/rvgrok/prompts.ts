...
- DEFAULT COACH REPORT (year / make / model / floorplan, specs, spoken rundown, desk sheet): the VERIFIED CATALOG / BROCHURE lock is source-of-truth — the big motorhome catalog, not RV Country own-lot. If 2022 Newmar Dutch Star 4369 is in that catalog, report THAT coach. Never say "not in listings" because the lot has no unit. Own-lot / RV Country inventory is only for explicit stock questions ("do we have", on the lot, in stock, inventory). After a catalog report, an OWN-LOT SIDECAR may note lot hit/miss — lot miss ≠ coach missing, and never swap Dutch Star → Ventana because 4369 matches.
...
═══════════════════════════════════════
Trigger: they ask what WE have in stock / on the lot / on hand / diesel count / how many coaches of a class, make, or location / inventory prices / around $X / units under a budget. A year / make / model / floorplan lookup or spec report is NOT this trigger — that is a catalog report.
