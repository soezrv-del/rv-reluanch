...
/**
 * RVFAX Catalog Daily Sync (v6 — next-year + last-year + 2010–present gaps)
 * -------------------------------------------------------------------------
 ...
 *
 * Responsibilities:
 * 1. Discover new / updated model lines for NEXT model year, CURRENT year, and PRIOR year
 *    across priority makes. (RV model years launch mid-calendar-year.)
...
const PRIORITY_MAKES = [
  // Premium / high-volume motorized
  'Newmar', 'Tiffin', 'Winnebago', 'Entegra Coach', 'Fleetwood',
  'Holiday Rambler', 'Dynamax', 'Renegade RV', 'Thor', 'American Coach',
  // High-volume towables & Class B/C
  'Grand Design', 'Airstream', 'Jayco', 'Keystone', 'Forest River',
  'Coachmen', 'Heartland', 'Dutchmen', 'Alliance RV', 'Palomino',
  'Lance', 'Outdoors RV', 'Nexus RV', 'Brinkley',
  // Specialty Class B / van
  'Leisure Travel Vans', 'Roadtrek', 'Pleasure-Way', 'Storyteller Overland',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── 2. New-model discovery — next + current + prior model year ────────
    // Next year first: by mid-calendar-year, MY+1 brochures are already shipping.
    const targetYears = [CURRENT_YEAR + 1, CURRENT_YEAR, CURRENT_YEAR - 1];
