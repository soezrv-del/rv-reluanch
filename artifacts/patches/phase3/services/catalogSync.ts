 ...
 *
 * Phase 3: waits for ensureCatalogLoaded() so the lean cold-start facade is
 * filled before remote rows are merged.
 */
...
let inFlight: Promise<{ added: number; updated: number; lastRefresh: string | null }> | null =
  null;
...
export async function refreshCatalogFromServer(): Promise<{
  ...
  inFlight = (async () => {
    try {
      // Local seed first, then merge remote rows on top
      ...
      for (const row of (data as CatalogRow[]) || []) {
        ...
        const lengthRange =
          row.length_range && row.length_range.length === 2
            ? (row.length_range as [number, number])
            : existing?.lengthRange || [0, 0];
        const weightRange =
          row.weight_range && row.weight_range.length === 2
            ? (row.weight_range as [number, number])
            : existing?.weightRange || [0, 0];
        const msrpRange =
          row.msrp_range && row.msrp_range.length === 2
        ...
        if (!existing) {
          ...
          Object.assign(RV_DATA[row.make][row.model], {
            ...
            ...(row.msrp_range &&
              row.msrp_range.length === 2 && { msrpRange }),
            ...(row.length_range &&
              row.length_range.length === 2 && { lengthRange }),
            ...(row.weight_range &&
              row.weight_range.length === 2 && { weightRange }),
      ...
      console.log(
        `[catalogSync] Merged remote catalog — added ${added}, updated ${updated}`,
      );
