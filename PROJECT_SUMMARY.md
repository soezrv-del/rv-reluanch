...
- **Facts videos:** YouTube Data API (`YOUTUBE_API_KEY` on Vercel Production + Preview, server-only); opt-in `GET /api/rv-videos` after “Want a video?” — never on Facts open
