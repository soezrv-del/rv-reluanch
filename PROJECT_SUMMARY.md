...
- **RV Grok:** xAI API (`XAI_API_KEY` on Vercel) for chat + Live Voice; optional Gemini Google Search grounding (`GEMINI_API_KEY` on Vercel Production + Preview) for the browse sidecar only (`RVGROK_RESEARCH_PROVIDER=auto` default). Chat via `/api/rvgrok`; voice web research via `/api/rvgrok/web-research`
