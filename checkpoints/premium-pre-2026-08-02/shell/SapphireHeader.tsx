...
export const PAGE_ACCENT: Record<
  AppTab,
  "sapphire" | "ruby" | "gold"
> = {
  rvfax: "sapphire",
...
export function SapphireHeader({ tab }: { tab: AppTab }) {
  ...
  const accent = PAGE_ACCENT[tab] ?? "sapphire";
  const isGrok = tab === "rvgrok";
  const isCal = tab === "rvcal";

  const shellGlow =
    accent === "ruby"
      ? {
          background:
            "linear-gradient(165deg, rgba(28,6,12,0.86) 0%, rgba(40,10,18,0.8) 40%, rgba(12,4,8,0.9) 100%)",
          boxShadow:
            "0 16px 48px rgba(40,0,10,0.55), inset 0 1px 0 rgba(255,180,190,0.22), inset 0 -1px 0 rgba(120,20,40,0.25)",
        }
      : accent === "gold"
        ? {
            background:
  ...
  const badgeTone =
    accent === "ruby"
      ? "border-ruby/40 bg-ruby/15 text-white"
      : accent === "gold"
      ...
      >
          ...
          style={{
            background:
              accent === "gold"
                ...
                : accent === "ruby"
                  ? "radial-gradient(ellipse 80% 60% at 50% 0%, rgba(212,37,53,0.2) 0%, transparent 55%)"
        ...
        <div className="relative z-[2] flex flex-col items-center text-center">
          {copy.badge ? (
            <span
              className={cn(
                "sapphire-header-badge mb-1.5 inline-flex items-center rounded-full border px-2.5 py-0.5 text-[8px] font-bold tracking-[0.22em]",
                badgeTone,
              )}
            >
              {copy.badge}
          ...
          <div className="sapphire-title-stage relative inline-flex max-w-full items-center justify-center overflow-visible px-2 py-1">
            <span
              aria-hidden
              className="sapphire-title-ambient pointer-events-none absolute -inset-x-8 -inset-y-4 z-0"
            />
            <span
              aria-hidden
            ...
            <h1
              className={cn(
                "sapphire-header-title sapphire-title-3d relative z-[2] max-w-[20ch] font-bold leading-[0.92] tracking-tight sm:max-w-none",
                isGrok
                  ? "ice-text-ruby ice-text-live-ruby"
                  : isCal
                    ? "ice-text-gold"
            ...
            <span
              aria-hidden
              className={cn(
                "sapphire-title-sheen pointer-events-none absolute inset-0 z-[3] flex items-center justify-center font-bold leading-[0.92] tracking-tight",
                copy.title.length <= 8
                  ? "text-[clamp(3rem,13vw,4.1rem)]"
                  : "text-[clamp(2.35rem,10.5vw,3.35rem)]",
              )}
